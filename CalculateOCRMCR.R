...
CalculateOCRMCR <- function(filename_pred, filename_conf, indexFile){
	...
	cat('Threshold = ', thresholdValues[i],'\n')
	 ...
	 classDex[which(newPredictions == "unclassified"),'Unclassified'] <- TRUE
 	 ...
 	 for(k in seq_along(predictions)) {
 	 	 	if(is.na(classDex[k,'Prediction']) == FALSE){


	 	 	 	if((classDex[k,'Actual'] %in% singletonGenera)){
	 	 	 		if(classDex[k,'Unclassified'] == FALSE){
	 	 	 			classDex[k,'OC'] <- 1
	 	 	 		}
	 	 	 	}
	 	 	 	else if((classDex[k,'Unclassified'] == FALSE)) {
	 	 	 		if(classDex[k,'Prediction'] != classDex[k,'Actual']){
	 	 	 			classDex[k,'MC'] <- 1
	 	 	 		}
 	 ...
 	 mcv <- sum(mcv)/lengthOfKnown
 	 MCvector[i] <- mcv
 	 cat('MC Rate at ', thresholdValues[i]*100,'% = ', MCvector[i],'\n')
	 cat('OC Rate at ', thresholdValues[i]*100,'% = ', OCvector[i],'\n')


  }
	return(list(mcv = MCvector, ocv = OCvector, uncl = unclassifiedVector))
}
