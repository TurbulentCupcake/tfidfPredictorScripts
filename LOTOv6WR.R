...
# --------------------------  THE ALGORTHM -----------------------------------
	...
	predictionVector <- vector(mode = 'character', length = length(testingSeqsIndices))
	...
	savelink <- paste(c('confidence_',end,'_v6_WRBalanced.RData'), collapse = "")
	savelink2 <- paste(c('predictions_',end,'_v6_WRBalanced.RData'), collapse = "")
