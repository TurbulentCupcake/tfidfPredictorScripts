...
# --------------------------  THE ALGORTHM -----------------------------------
	...
	{	
		...
		# In this version, our confidence is simply determined by the quality of our 
		# bootstraps. It takes into account assigning a confidence simply as 
		# the di/ davg and take this value for everything. 
		...
		if(predictions[i] %in% predictedRanks) { 
				...
				confidence <- sum(sequence_df[which(sequence_df[,1] == predictions[i]),4])
	...
	savelink <- paste(c('SINTAXtfidf',k,'mers_',end,'_v2.RData'), collapse = "")
