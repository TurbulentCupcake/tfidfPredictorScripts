...
# Load an index file that will load according to what subset you enter. 
indexFile <- paste0(c(subset,'Upindex.RData'), collapse = '')
load(indexFile)
...
# Before we send the sequences in, we need to preprocess the sequences set and index 
# for those elements that fall within the specified sequence length. 
...
for(i in 1:length(sequences)){
	# picking only those sequences greater than the subset
	if(nchar(sequences[[i]]) > subset){
		# we can pick any subset of strings of length that of size of the subset
		overflow = nchar(sequences[[i]]) - subset
		# pick random number less than overflow - 1
		startindex = sample(overflow)[1]
		# now pick our new subset of the sequence
		seqs[[i]] <- substr(sequences[[i]], startindex, startindex+subset)
		# adding the index
		# index[[i]] <- i
	}
}
...
# index <- unlist(index)
# now we need to remove null values in both the lists

testingSequences <- seqs
trainingSequences <- origSequences
testingRanks <- rank[index]
trainingRanks <- rank


trainMers <- lapply(trainingSequences,
	function (x) {
		substring(x,
			1:(nchar(x) - k + 1L),
			k:nchar(x))
	})
names(trainMers) <- rank


testMers <- lapply(testingSequences,
	function (x) {
		substring(x,
			1:(nchar(x) - k + 1L),
			k:nchar(x))
	})
names(testMers) <- rank[index]
...
# --------------------------  THE ALGORTHM -----------------------------------
	...
	bs_confidence_vector <- vector(mode = 'integer', length=length(testMers))
	names(bs_confidence_vector) <- rank[index]
	predictionVector <- vector(mode = 'character', length = length(testMers))
	...
	{
		testSeq <- testMers[[i]]
		testRank <- testingRanks[[i]]
		# predictedRankFromPredictions <- predicted_RDP_sintax[i]
		training_db_rank <- trainingRanks[-index[i]]
		training_db_seqs <- trainMers[-index[i]]
		...
		samp_matrix_w <- matrix(sample(length(testSeq),s*100,replace = TRUE),nrow=100,ncol=s)
		...
		for(j in 1:100) {
			...
			sequence_df[j,2] <- hi/s
			cat('Calculated bootstrap value : ', sequence_df[,2],'\n')
		}
	...
	savelink <- paste(c(s,'_',subset,'_',end,'_confidencev2.RData'), collapse = "")
	savelink2 <- paste(c(s,'_',subset,'_',end,'_predictionv2.RData'), collapse = "")
